...
bash supabase/tests/run.sh   # Host-mutation migrations against a throwaway Postgres in Docker
```
...
- **Recording a winner is idempotent on a client-supplied claim key.** The host client mints a uuid when the Record Winner modal opens and sends it as `clientRequestId`; it lands in `winners.client_request_id`, which has a unique index where not null. A retry of the same claim carries the same key, so the function inserts nothing and returns the current state instead of raising. A tie is a separate claim with a separate key, so both winners still save. Never key on `(game_id, stage)` or `call_count_at_win`: ties make those legitimately non-unique. See `supabase/migrations/20260730064309_winner_idempotency_key.sql` and `src/lib/claim-request-id.ts`.
...
| `winners` | Audit row per recorded win (`winner_name = 'Anonymous'`, `client_request_id` unique where not null) |
...
- Host actions re-verify auth and check `profiles.role`. The privileged writes go through `security definer` Postgres functions called with the **cookie-based** client (they read `auth.uid()`), not the service-role client.
- **Admin vs host boundary on `winners`.** RLS: SELECT is public, INSERT is admin **or** host, UPDATE is **admin only**. That split is deliberate: voiding a win is a money decision (admin), marking a prize handed over is housekeeping done by whoever holds the prize (host). Hosts reach `prize_given` through `set_winner_prize_given`, which writes that one column and nothing else. Do **not** widen the `winners` UPDATE policy to hosts: a host holds a real JWT in the browser and could then `PATCH /rest/v1/winners` to set `is_void` directly, walking past the admin-only refusal in `voidWinnerFromHost`.
...
- Existing coverage: `connection-health`, `game-state-version`, `log-error`, `prize-validation`, `win-stages`, `claim-request-id`, plus a handful of small utility tests.
- Mock Supabase in the Node tests. Never hit a real database from them.
- Postgres functions are tested separately by `bash supabase/tests/run.sh`, which builds a throwaway `postgres:17` container (Docker plus `psql` required), applies the host-mutation migrations in order over a populated `winners` table, and asserts behaviour including retries, ties, contention across two live connections, and grants. It exits non-zero on failure, so a production apply can be gated on it. It never connects to a real project.
...
- **Don't mint a fresh claim key per attempt.** `claimRequestIdRef` is set when the Record Winner modal opens and must stay put for every tap inside it, retries included. Regenerating it in the submit handler, or clearing it in the `catch`, silently removes all duplicate-winner protection while looking tidier. Fresh keys belong at modal-open only, which is also how a genuine tie gets its own.
- **Never report success on an update you have not proved landed.** A Supabase `.update()` without `.select()` that RLS filters out returns **no error and no rows**, so `if (error)` passes and the action reports success while nothing was written. That is what made a host's "prize given" tick a lie, and the same shape had already bitten the snowball pot. Every direct `.update()` must either `.select()` and treat zero rows as an error, or go through an RPC that returns the persisted value.
